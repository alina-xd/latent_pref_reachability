...
class DubinsCarAvoidLatentEnv(gym.Env):
  ...
  def __init__(
    ...
    # State bounds.
    self.bounds = np.array([[-1.5, 1.5], [-1.5, 1.5], [0, 2 * np.pi]])
  ...
  def capture_image(self, state=None):
    """Render an observation for state [x, y, theta] using the SAME renderer as
    the world-model training data (gray sidewalks + red obstacle + tree +
    top-down car icon), so the WM encoder sees in-distribution images."""
    if state is None:
      state = self.state
    if not hasattr(self, "_gen"):
      import sys
      import pathlib
      repo = str(pathlib.Path(__file__).resolve().parents[5])  # latent_pref_reachability
      if repo not in sys.path:
        sys.path.insert(0, repo)
      import generate_data_dubins_sidewalk as gen
      self._gen = gen
      self._emoji = gen.load_tree_emoji()
    return self._gen.render_state(
        np.asarray(state, dtype=np.float32),
        self.speed, self.time_step, self._emoji, self.image_size)
  # == Plotting Functions ==
  def render(self):
    pass
  ...
  def visualize(
    ...
    Args:
        ...
        num_rnd_traj (int, optional): #trajectories. Defaults to None.
    """
    # Eval plot: learned safety value V(x) = max_a Q(x, a).
    #   top row    : continuous value (seismic, >0 safe / <0 unsafe)
    #   bottom row : discrete safe set {V > 0}
    # Overlay is the true sidewalk failure set (obstacle circle + gray sidewalk
    # strips) -- NOT the stale LS_BRT center-only grid contour / confusion score.
    from matplotlib.patches import Circle as _Circle
    try:
      import generate_data_dubins_sidewalk as _gen
      SW = _gen.SIDEWALK_Y
    except Exception:
      SW = 1.1
    thetaList = [0, np.pi / 2, np.pi, np.pi * 3 / 2]
    ext = self.get_axes()[0]
    xlo, xhi = self.bounds[0, 0], self.bounds[0, 1]
    ylo, yhi = self.bounds[1, 0], self.bounds[1, 1]
    fig, axes = plt.subplots(2, len(thetaList), figsize=(4 * len(thetaList), 8))
    vgrids = [self.get_value(q_func, theta, nx, ny, addBias=addBias) for theta in thetaList]
    vlim = max(max(abs(v).max() for v in vgrids), 1e-3)   # symmetric, data-driven (raw margin range)
    imc = None
    for c, theta in enumerate(thetaList):
      v = vgrids[c]
      imc = axes[0, c].imshow(
          v.T, interpolation="none", extent=ext, origin="lower",
          cmap=cmap, vmin=-vlim, vmax=vlim, zorder=-1)
      axes[0, c].set_title(
          r"$V(x)$   $\theta={:.0f}^\circ$".format(theta * 180 / np.pi), fontsize=16)
      axes[1, c].imshow(
          v.T > 0, interpolation="none", extent=ext, origin="lower",
          cmap=cmap, vmin=-1, vmax=1, zorder=-1)
      axes[1, c].set_xlabel(
          r"$\theta={:.0f}^\circ$".format(theta * 180 / np.pi), fontsize=18)
      for ax in (axes[0, c], axes[1, c]):
        self.plot_formatting(ax=ax, labels=labels)
        ax.add_patch(_Circle((0, 0), self.config.obs_r, fill=False, color="k", lw=2, zorder=5))
    ...
    axes[0, 0].set_ylabel("continuous value", fontsize=15)
    axes[1, 0].set_ylabel(r"discrete BRT $\{V>0\}$", fontsize=15)
    plt.tight_layout()
