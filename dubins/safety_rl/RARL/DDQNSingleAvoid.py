...
class DDQNSingleAvoid(DDQNAvoid):
  ...
  def update(self, addBias=False):
    ...
    if self.mode == "RA" or self.mode =='avoid':
      ...
      # TODO Q2.2: Implement the discounted reach-avoid Bellman equation
      # This implements the discounted reach-avoid Bellman equation: V(x) = (1 - gamma) * g(x) + gamma * min{g(x), max_u V(f(x,u))}
      # Hint: Use self.GAMMA, g_x, and state_value_nxt.
      # y = (1 - self.GAMMA) * ... + self.GAMMA * ...
      y = (1 - self.GAMMA) * g_x + self.GAMMA * torch.min(g_x, state_value_nxt)
