...
class WorldModel(nn.Module):
    def __init__(self, obs_space, act_space, step, config):
        ...
        # Optional margin head: regresses the (weighted signed-distance) failure
        # margin jointly with the world model so the latent is shaped by it.
        if getattr(config, "use_margin_head", False):
            self.heads["margin"] = networks.MLP(
                ...
                config.act,
                config.norm,
                dist=config.margin_head["dist"],
                outscale=config.margin_head["outscale"],
                device=config.device,
                name="Margin",
            )
        ...
        if getattr(config, "use_margin_head", False):
            self._scales["margin"] = config.margin_head["loss_scale"]
